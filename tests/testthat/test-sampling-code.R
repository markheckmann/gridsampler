...
test_that("Converges to theoretical MWNHD for fixed number of attributes", {
  ...
  for (i in 1:10) {
    ...
    f <- sim_n_persons(P, n = 1e6, a = N)   # frequency of each category
...
test_that("Covergence to MWNHD for random number of attributes", {
  ...
  for (i in 1:10) {
    ...
    f <- sim_n_persons_x_times(P, n = 1e6, a = NN, ap = ap, times = 1, progress = "none")
    h <- f / sum(f)                     # relative frequencies
